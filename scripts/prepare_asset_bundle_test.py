...
def handoff():
    ...
    return {
        ...
        "state_path": ".github/assets/release-state.json",
    }


def write_release_archive(path, release):
    with tarfile.open(path, "w:gz") as archive:
        for directory in ("releases/", f"releases/{release}/"):
            info = tarfile.TarInfo(directory)
            info.type = tarfile.DIRTYPE
            archive.addfile(info)
        for filename in ("release.json", "checksums.txt"):
            contents = b"{}\n"
            info = tarfile.TarInfo(f"releases/{release}/{filename}")
...
class AssetHandoffContractTest(unittest.TestCase):
    ...
    def test_rejects_cross_schema_and_malicious_urls(self):
        ...
        bad_channel_id["channel_artifact_id"] = "0123456"
